...
# Patterns for splitting a compound "compare X and Y" query into two sub-queries
_COMPARE_AND = re.compile(
    r"(?:compare|contrast|difference\s+between|differences?\s+between)\s+"
    r"(.+?)\s+and\s+(.+?)(?:\s*[,;:]|\s+(?:in|for|over|during|by|from|as)\b|$)",
    re.IGNORECASE,
)
...
def should_decompose(
    ...
    # Gate 2: Already inside a structural decomposition — hard block
    if _structural_depth.get() > 0:
        return False
...
def _extract_tasks(
    ...
    return (
        ...
        or _try_entity_split(query, conversation_id)
