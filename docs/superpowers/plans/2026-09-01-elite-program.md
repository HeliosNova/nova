...
- Task 4.6: Re-run the priming A/B (n=16) in a quiet window with the fixed key — needs an owner-chosen quiet window (pauses the 39 domain monitors for hours). Recipe: `docker exec nova-app python scripts/ceiling_ab.py --help` (passes `dossier_key=monitor` since Phase 1.2).
