...
class TestConfigUpdateArchivedFields:
    """PATCH /api/config must REJECT the archived multi-agent fields.

    The decomposition capability was archived 2026-08-25 (its flags removed
    from config + the pydantic request model, extra=forbid) — accepting the
    old knobs silently would fake a capability that no longer exists.
    """

    def test_archived_multi_agent_fields_rejected(self, client):
        resp = client.patch("/api/config", json={"ENABLE_MULTI_AGENT": True})
        assert resp.status_code == 422

    def test_archived_agent_count_rejected(self, client):
